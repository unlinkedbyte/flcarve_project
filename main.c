...
int main (int argc, char *argv[]) {
  ...
  unsigned char antepenultimate_byte = 0x00;
  ...
  while ((bytes_read = fread(buffer, 1, sizeof(buffer), disk_image)) > 0) {
    ...
    if (bytes_read < 4) {
      if (carving == 1) {
        ...
        fprintf(stderr, "[-] unknown carving state: %d\n", carving);
        fclose(disk_image);
        return EXIT_FAILURE;
      }
    } else {
      ...
      if (!first_iteration)  {
        ...
        if (antepenultimate_byte == 0x1F && penultimate_byte == 0x8B && last_byte == 0x08 && (buffer[0] & 0xE0) == 0) {
          printf("[+] Gzip signature found on the block boundary!\n");
          printf("  Absolute disk offset: %zu (0x%zX)\n", global_offset -3, global_offset - 3);

          if (carving == 1 && output_file != NULL) {
            fflush(output_file);
            off_t file_size = ftello(output_file);
            ftruncate(fileno(output_file), file_size - 3);
            fclose(output_file);
          }
          ...
          fwrite(&antepenultimate_byte, 1, 1, output_file);
          fwrite(&penultimate_byte, 1, 1, output_file);                                                                                                                                
          fwrite(&last_byte, 1, 1, output_file);  

        }

        // Bloque añadido: nueva comprobación con la misma lógica.
        if (penultimate_byte == 0x1F && last_byte == 0x8B && buffer[0] == 0x08 && (buffer[1] & 0xE0) == 0) {
          printf("[+] Gzip signature found on the block boundary!\n");
  ...
  */
        ...
        if (last_byte == 0x1F && buffer[0] == 0x8B && buffer[1] == 0x08 && (buffer[2] & 0xE0) == 0) {
      ...
      for (size_t i = 0; i < bytes_read - 3; i++) { // Aquí el problema de que i < bytes_read -2 siempre sea 4094(4093) y nunca lleguemos a los dos bytes del final, por lo que hay que poner una nueva lógica
        ...
        if (buffer[i] == 0x1F && buffer[i+1] == 0x8B && buffer[i+2] == 0x08 && (buffer[i+3] & 0xE0) == 0) {
      ...
      antepenultimate_byte = buffer[bytes_read -3];
